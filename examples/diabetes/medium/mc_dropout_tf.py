...
# Numbers to run evaluation with stochastic metrics
NUM_EVALUATIONS = 10
VAL_REPORT_FREQ = 1
...
def model_fn(features, labels, mode, params):
  with tf.variable_scope('model',
    ...
    t = tf.layers.Conv2D(initial_conv_units,
                         3,
                         strides=(2, 2),
    ...
    t = tf.layers.Conv2D(initial_conv_units * 2,
                         3,
                         strides=(1, 1),
                         activation=leaky_relu,
                         padding="same",
    ...
    t = tf.layers.Conv2D(initial_conv_units * 4,
                         3,
                         strides=(1, 1),
                         activation=leaky_relu,
                         padding="same",
    ...
    t = tf.layers.Conv2D(initial_conv_units * 8,
                         3,
                         strides=(1, 1),
                         activation=leaky_relu,
                         padding="same",
    ...
    loss_vec = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(
    ...
    accuracy = tf.metrics.accuracy(labels=labels, predictions=tf.to_int64(pred_sig > 0.5))
    accuracy_per_class = tf.metrics.mean_per_class_accuracy(labels=labels,
...
def main(argv):
  ...
  for i in range(1, 300, 3):
    ...
    classifier.evaluate(input_fn=dtask.validate_tf_input_fn(
        batch_size=BATCH_SIZE, nb_workers=20))
    ...
    #     tex = report.to_latex(
    #         output_file=
    #         "tmp/results/diabetes/medium/mc_dropout_tf/val_epoch{}_report.tex".
    #         format(i),
    #         title="Diabetes medium benchmark with MC Dropout baseline")
    #     print(tex)
    #   except ValueError:
    #     print("ValueError in generate report, likely due to only one class present in y_true.")
    #     pass
  ...
  for n in range(NUM_EVALUATIONS):
    ...
    if n == NUM_EVALUATIONS - 1:
        # Generate report
        tex = report.to_latex(
            output_file='tmp/results/diabetes/medium/mc_dropout_tf/report.tex'.
            format(n=n),
            title='Diabetes medium benchmark with MC Dropout baseline')
        print(tex)
